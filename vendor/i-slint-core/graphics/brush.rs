    ...
    pub fn color(&self) -> Color {
        match self {
            ...
            Brush::LinearGradient(gradient) => gradient
                ...
                .map(|stop| stop.color)
                .unwrap_or_default(),
            Brush::RadialGradient(gradient) => gradient
                .stops()
                .next()
                .map(|stop| stop.color)
                .unwrap_or_default(),
            Brush::ConicGradient(gradient) => gradient
                .stops()
                .next()
                .map(|stop| stop.color)
                .unwrap_or_default(),
...
impl ConicGradientBrush {
    ...
    fn normalize_stops(&mut self) {
        ...
        let has_stop_at_1 = stops_slice
            .iter()
            .any(|s| (s.position - 1.0).abs() < f32::EPSILON);
        let has_stops_outside = stops_slice
            .iter()
            .any(|s| s.position < 0.0 || s.position > 1.0);
...
pub fn line_for_angle(angle: f32, size: Size2D<f32>) -> (Point2D<f32>, Point2D<f32>) {
    ...
    let (a, b) = if s.abs() < f32::EPSILON {
        ...
        (
            Point2D::new(x, y),
            Point2D::new(size.width - x, size.height - y),
        )
    } else {
...
fn test_linear_gradient_encoding() {
    let stops: SharedVector<GradientStop> = [
        GradientStop {
            position: 0.0,
            color: Color::from_argb_u8(255, 255, 0, 0),
        },
        GradientStop {
            position: 0.5,
            color: Color::from_argb_u8(255, 0, 255, 0),
        },
        GradientStop {
            position: 1.0,
            color: Color::from_argb_u8(255, 0, 0, 255),
        },
    ]
    .into();
...
fn test_conic_gradient_stops_outside_range() {
    ...
    let grad = ConicGradientBrush::new(
        ...
        [
            ...
            GradientStop {
                position: 1.2,
                color: Color::from_rgb_u8(0, 0, 255),
            },
        ],
    );
